...
3. 金額は「合計」「現金計」「お買上計」などと書かれた行の数字を真の合計として基準にし、各商品の金額の合計と一致するか頭の中で検算してから出力すること。
4. 数字は 1 桁も間違えないように、画像を細部まで(にじみ・かすれ・小さい文字も含めて)拡大するつもりで確認すること。1 と 7、3 と 8、0 と 6 などの取り違えに特に注意する。
5. 読み取れない情報は無理に推測しないこと。日付や店舗名が読めない場合は文字列 "unknown" を返す。商品行で価格が読み取れないものは、その商品行ごと items から除外する。

【その他の注意事項】
- 値引き・小計・消費税・合計・現金計などの集計行は items に含めず、購入した商品の行のみを items に入れること。
- price は半角数値のみ(カンマや円マーク、税表示記号は含めない)。
